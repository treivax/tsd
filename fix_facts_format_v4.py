...
def fix_facts_format_extended_v2():
    ...
    for filename in os.listdir(extended_dir):
        if filename.endswith('.facts'):
            ...
            # Séparer par lignes et reconvertir
            lines = content.split('\n')
            new_lines = []
...
if __name__ == "__main__":
    print("🔧 CORRECTION FORMAT FICHIERS .facts ÉTENDUS V2")
    print("===============================================")
    
    corrected = fix_facts_format_extended_v2()
    
    print(f"\n📊 Résultats:")
    print(f"   • {corrected} fichiers corrigés")
    print(f"   • Format: Type{{key: value}} → Type[key=value]")
    print("   • Retours de ligne corrigés")
    print("\n✅ Correction terminée !")