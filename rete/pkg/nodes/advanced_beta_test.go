...
func TestNotNode_EvaluateCondition_InvalidCondition(t *testing.T) {
	logger := newMockLogger()
	node := NewNotNode("not1", logger)
	...
	result, err := node.evaluateCondition(condition, token, fact)
	if err != nil {
		t.Errorf("evaluateCondition should not return error for invalid type: %v", err)
	}
	if result {
		t.Error("evaluateCondition should return false for invalid condition")
	}
}

func TestNotNode_EvaluateBinaryCondition_MissingOperator(t *testing.T) {
	logger := newMockLogger()
	node := NewNotNode("not1", logger)
	...
	result, err := node.evaluateBinaryCondition(condition, token, fact)
	if err == nil {
		t.Error("evaluateBinaryCondition should return error when operator is missing")
	}
	if result {
		t.Error("evaluateBinaryCondition should return false on error")
	}
}

func TestNotNode_EvaluateBinaryCondition_AllOperators(t *testing.T) {
	logger := newMockLogger()
	node := NewNotNode("not1", logger)
	...
	tests := []struct {
		...
		{"less than true", "<", 20, 30, true},
		{"less than false", "<", 30, 20, false},
		{"less or equal true", "<=", 25, 25, true},
		{"less or equal false", "<=", 30, 25, false},
		{"greater than true", ">", 30, 20, true},
		{"greater than false", ">", 20, 30, false},
...
func TestNotNode_ExtractFieldValue_Missing(t *testing.T) {
	logger := newMockLogger()
	node := NewNotNode("not1", logger)
	...
	leftExpr := map[string]interface{}{
		"field": "age", // Field doesn't exist in fact
	}

	_, err := node.extractFieldValue(leftExpr, token, fact)
	if err == nil {
		t.Error("extractFieldValue should return error for missing field")
	}
}

func TestNotNode_ExtractFieldValue_EmptyToken(t *testing.T) {
	logger := newMockLogger()
	node := NewNotNode("not1", logger)
...
func TestNotNode_ExtractConstantValue(t *testing.T) {
	logger := newMockLogger()
	node := NewNotNode("not1", logger)
	...
	tests := []struct {
		...
		{
			name:      "float value",
			rightExpr: map[string]interface{}{"value": 3.14},
			wantValue: 3.14,
			wantErr:   false,
		},
		{
			name:      "missing value - returns map directly",
			rightExpr: map[string]interface{}{"other": 42},
			wantValue: nil, // We'll check it's a map separately
			wantErr:   false,
		},
		{
			name:      "direct value string",
			rightExpr: "not_a_map",
			wantValue: "not_a_map",
			wantErr:   false,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			value, err := node.extractConstantValue(tt.rightExpr)
			...
			if !tt.wantErr {
				...
				if tt.name == "missing value - returns map directly" {
					...
					t.Errorf("extractConstantValue() = %v, want %v", value, tt.wantValue)
				}
			}
		})
	}
...
func TestNotNode_CompareValues_DifferentTypes(t *testing.T) {
	logger := newMockLogger()
	node := NewNotNode("not1", logger)

	// Compare int with string using == (converts to string)
	result, err := node.compareValues(42, "==", "42")
	if err != nil {
	...
	result2, err2 := node.compareValues(42, "<", "not_a_number")
	if err2 == nil {
		t.Error("compareValues should return error for < with non-numeric string")
	}
	if result2 {
		t.Error("compareValues should return false on error")
	}
}

func TestNotNode_CompareValues_StringComparison(t *testing.T) {
	logger := newMockLogger()
	node := NewNotNode("not1", logger)
	...
	tests := []struct {
		...
		{"string equal", "test", "==", "test", true, false},
		{"string not equal", "test", "!=", "other", true, false},
		// Note: < and > on strings require numeric conversion which will fail
		{"string less than (error)", "apple", "<", "banana", false, true},
		{"string greater than (error)", "zebra", ">", "apple", false, true},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result, err := node.compareValues(tt.left, tt.operator, tt.right)
			if (err != nil) != tt.wantErr {
...
func TestNotNode_CompareValues_FloatComparison(t *testing.T) {
	logger := newMockLogger()
	node := NewNotNode("not1", logger)

	result, err := node.compareValues(3.14, "<", 3.15)
	if err != nil {
...
func TestNotNode_CompareValues_InvalidOperator(t *testing.T) {
	logger := newMockLogger()
	node := NewNotNode("not1", logger)

	result, err := node.compareValues(42, "INVALID", 42)
	if err == nil {
