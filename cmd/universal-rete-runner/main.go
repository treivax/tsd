	...
	for i, testFile := range testFiles {
		...
		if testResult.Passed {
			...
			if errorTests[testFile.Name] {
				...
				if testResult.Error != nil {
					fmt.Fprintf(stdout, " - Error: %v", testResult.Error)
				}
				fmt.Fprintf(stdout, " - network=%v, facts=%d, hasInjectionErrors=%v\n",
					testResult.TerminalNodes > 0, testResult.Facts,
					strings.Contains(testResult.Output, "⚠️ Erreur injection fait"))
			}
			result.Failed++
		}
	}
...
func ExecuteTest(testFile TestFile, expectError bool) TestResult {
	...
	// Simply ingest the .tsd file - it contains both constraints and facts
