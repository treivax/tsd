#!/bin/bash
# Review Script - Exécution séquentielle avec GitHub Copilot CLI
# Applique review.md sur chaque SESSION*.md du répertoire
set -euo pipefail
...
# Lister les fichiers SESSION*.md par ordre lexicographique
get_session_files() {
    find "$SCRIPT_DIR" -maxdepth 1 -name "SESSION*.md" -type f | sort
}
...
main() {
    ...
    if [ $total_sessions -eq 0 ]; then
        echo -e "${RED}❌ Aucun fichier SESSION*.md trouvé dans $SCRIPT_DIR${NC}"
        exit 1
    fi
...
show_help() {
    ...
    echo "  $0 -y                   Automatique sans confirmation"
    echo "  $0 -y -c                Automatique + continue sur erreur"
    echo "  $0 -p 30                Pause 30s entre sessions"
    echo ""
    echo "  AUTO_CONFIRM=1 $0       Variable d'environnement"
}
